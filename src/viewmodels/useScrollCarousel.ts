...
// Carousel expand constants
const W_START = 60, W_END = 94   // frame width %
const R_START = 20, R_END = 4    // border-radius px
...
export function useScrollCarousel(): CarouselViewModel {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      outer.style.height = window.innerHeight * (N + 0.6) + 'px'
    }
    resize()
    ...
    const onScroll = () => {
      ...
      const progress = clamp((sy - top) / (outer.offsetHeight - vh), 0, 1)
      const eased    = 1 - Math.pow(1 - progress, 3)

      frame.style.width        = (W_START + (W_END - W_START) * eased) + '%'
      frame.style.borderRadius = (R_START + (R_END - R_START) * eased) + 'px'
